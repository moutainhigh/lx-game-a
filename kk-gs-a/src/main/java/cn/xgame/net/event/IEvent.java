...
public abstract class IEvent {
	 ...
	 * @throws IOException 
	 */
	public abstract void run( Player player, ByteBuf data ) throws IOException;
...
//			Logs.error( "发包时出错  con=null！ （可能原因服务端还没发包时客户端就已经断开连接了）" );
	 ...
	 * @return
	 */
	protected ByteBuf buildEmptyPackage( AbstractChannel ctx,int capacity ){
		ByteBuf buffer = ctx.alloc().buffer( capacity + 6 );
		buffer.writeByte( Config.PACKAGE_HEAD );
		buffer.writeShort( packageNo );
		buffer.writeShort( (short) 0 );//长度占位符
	 ...
	 * 向客户端发送包
	 */
	public void sendPackage( AbstractChannel ctx, ByteBuf buffer ) throws IOException{
		buffer.setShort( 3, (short) (buffer.writerIndex() - 5) );//设置内容长度
		buffer.writeByte( Config.PACKAGE_FOOT );
